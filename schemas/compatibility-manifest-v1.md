...
| --- | --- | --- |
| `addon_version` | string | `"0.1.0"`; must equal the repository `VERSION` used for the build |
...
| `fight_spec_schema_version` | integer | `1`; validated `FightSpec` schema |
| `generator_version` | integer | `1`; deterministic generator behavior |
| `catalog_revision` | integer | `1`; exact catalog semantics |
| `layout_revision` | integer | `1`; exact layout semantics (called `layout_version` inside `FightSpec`) |
