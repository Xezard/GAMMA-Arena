...
local task6 = "gamma_arena_actor_adapter.new gamma_arena_checkpoint_adapter.new level.disable_input level.enable_input safe_release_manager.release drain_checkpoint_cleanup teardown_clock teardown_timeout_ms teardown_max_updates GA_RUNTIME_TEARDOWN_TIMEOUT GA_RUNTIME_TEARDOWN_EXHAUSTED"
...
local function teardown_retry(result) if result.ok then self.teardown_done = true end end
