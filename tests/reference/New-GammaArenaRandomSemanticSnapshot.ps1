...
$snapshotPath = Join-Path $repoRoot 'tests\fixtures\golden-random-selections-v9.txt'
...
if ([int]$catalog.meta.schema_version -ne 10 -or [int]$catalog.meta.revision -ne 11 -or [int]$catalog.meta.generator_version -ne 11 -or
    ...
    throw 'Random semantic reference requires catalog schema/revision/generator 10/11/11, difficulty 5, and layout 2 LTX inputs.'
}
...
if (@(Compare-Object @($deviceManifest.Id) $deviceIds -SyncWindow 0).Count -ne 0) { throw 'Device ID order differs from the exact v11 selection manifest.' }
for ($index = 0; $index -lt $deviceManifest.Count; $index++) {
    ...
    if ($configured.section -cne $deviceManifest[$index].Section -or [int]$configured.selection_weight -ne $deviceManifest[$index].Weight) {
        throw "Device $($deviceManifest[$index].Id) differs from the exact v11 selection manifest."
    }
}
...
function New-GaActorDevice([int64]$SessionSeed,[int]$FightIndex) {
    $draw = Get-GaInt (New-GaRng @('random_actor_device_v11',$SessionSeed,$FightIndex,11,2,'actor_device')) 1 100
    $cumulative = 0
    foreach ($device in $deviceManifest) {
        $cumulative += $device.Weight
        if ($draw -le $cumulative) { return $device.Section }
    }
    throw 'Exact actor-device weights exhausted their range.'
}
...
function New-GaSemantic([int64]$Seed,[string]$DifficultyId,[int]$FightIndex){
    ...
    $actorDevice = New-GaActorDevice $request.SessionSeed $FightIndex
    return "seed=$Seed,difficulty=$DifficultyId,fight=$FightIndex|actor=$($actor.Weapon),$($actor.Ammo),$($actor.AmmoBoxes),$($actor.Outfit),$actorKnife,medical:$($actorMedical-join'+'),bonus:$($actor.BonusSection):$($actor.BonusRequestedCategory):$($actor.BonusResolvedCategory):$($actor.BonusBoxes),device:$actorDevice|opponents=|$($opponents-join'|')"
}
...
if($Verify){if(-not(Test-Path -LiteralPath $snapshotPath)){throw 'Frozen random v9 semantic snapshot is missing.'};$actual=@(Get-Content -LiteralPath $snapshotPath|Where-Object{$_-and-not$_.StartsWith('#')});if(@(Compare-Object $expected $actual -SyncWindow 0).Count-ne0){throw 'Frozen random v9 semantic snapshot differs from independent LTX semantics.'};Write-Host 'PASS: frozen random v9 semantics match independent LTX reference'}elseif($Update){$lines=@('# Gamma Arena format-neutral random semantic selections v9.')+$expected;[IO.File]::WriteAllText($snapshotPath,($lines-join"`n")+"`n",(New-Object Text.UTF8Encoding($false)));Write-Host "Updated random v9 semantic snapshot: $snapshotPath"}else{$expected}
