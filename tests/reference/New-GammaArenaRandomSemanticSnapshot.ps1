...
function New-GaActorGear($Request,[int]$FightIndex,$Difficulty) {
    ...
    return [pscustomobject]@{Weapon=$weapon.Section;Ammo=$weapon.Ammo;AmmoBoxes=$boxes;Outfit=$outfit.Section}
}
...
function New-GaSemantic([int64]$Seed,[string]$DifficultyId,[int]$FightIndex){
    ...
    return "seed=$Seed,difficulty=$DifficultyId,fight=$FightIndex|actor=$($actor.Weapon),$($actor.Ammo),$($actor.AmmoBoxes),$($actor.Outfit),$actorKnife,medical:$($actorMedical-join'+')|opponents=|$($opponents-join'|')"
}
...
function ConvertFrom-GaV5Projection([string]$Line){
    ...
    if($encoded-notmatch'\|actor=([^|]+)\|opponents=\|(.*)\|diagnostic='){throw 'Malformed v5 semantic payload.'};$actorFields=$Matches[1].Split(',');$opponentText=$Matches[2];$actor="$($actorFields[2]),$($actorFields[3]),$($actorFields[4]),$($actorFields[5]),$($actorFields[6]),$($actorFields[7])"
