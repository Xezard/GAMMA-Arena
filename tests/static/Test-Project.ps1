...
$Task3ScriptContracts = @(
    ...
    , [PSCustomObject]@{ Path = 'dev\gamedata\scripts\gamma_arena_test_fight_spec.script'; Namespace = 'gamma_arena_test_fight_spec'; Required = @('(?m)^function\s+run\s*\(', 'fight_spec_canonicalizes_legacy_and_item_loadouts_equivalently', 'fight_spec_rejects_forbidden_keys_at_every_depth_and_invalid_identity', 'fight_spec_binds_identity_layout_and_dense_arrays', 'fight_spec_rejects_invalid_resolved_layout_identity_fields', 'fight_spec_uses_resolved_layout_id_and_aggregates_legacy_ammo') }
)
