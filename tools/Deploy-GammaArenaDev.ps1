...
$ExpectedTarget = Join-Path $ResolvedModsDirectory 'Gamma Arena DEV'
$TargetDirectory = [IO.Path]::GetFullPath($ExpectedTarget).TrimEnd('\', '/')

if (-not [string]::Equals($TargetDirectory, [IO.Path]::GetFullPath($ExpectedTarget).TrimEnd('\', '/'), [StringComparison]::OrdinalIgnoreCase)) {
...
if (Test-Path -LiteralPath $TargetDirectory) {
    ...
    if (-not $ResolvedTarget.StartsWith($ModsPrefix, [StringComparison]::OrdinalIgnoreCase) -or
        [string]::Equals($ResolvedTarget, $ResolvedModsDirectory, [StringComparison]::OrdinalIgnoreCase)) {
